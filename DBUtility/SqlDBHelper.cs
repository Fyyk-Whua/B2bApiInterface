...
namespace DBUtility
{
    ...
    public class SqlDBHelper : DBHelper
    {
        ...
        #region  ExecutePQuery 执行Transact-SQL语句并返回数据集DataTable
        /// <summary>
        /// ExecutePQuery
        /// </summary>
        ...
        #region BulkInsert 批量插入 SqlBulkCopy
        /// <summary>
        /// 批量插入 SqlBulkCopy
        /// </summary>
        public override bool BulkCopyInsert(DataTable dt, string rowguid, string TableName)
        {
            ...
            if (dt != null)
            {
                try
                {
                    DbConnection sqlcon = PrepareCon();
                    DbTransaction sqlbulkTransaction = sqlcon.BeginTransaction();
                    SqlBulkCopy bulk = new SqlBulkCopy((SqlConnection)sqlcon, SqlBulkCopyOptions.CheckConstraints, (SqlTransaction)sqlbulkTransaction);
